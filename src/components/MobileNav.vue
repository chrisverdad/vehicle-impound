<template>
  <div :class="['mobile-nav','no-print', { open: mobileMenuOpen }]">
    <div v-for="link in links" :key="link.page" class="nav-link" :class="{ active: page === link.page }" @click="navigate(link.page)">
      <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" v-html="link.icon"></svg>
      {{ link.label }}
    </div>
    <div class="mobile-nav-bottom">
      <div class="mobile-time">🕐 {{ currentTime }}</div>
      <div style="display:flex;align-items:center;justify-content:center;gap:8px;font-size:12px;color:var(--gray)">
        <span style="width:7px;height:7px;border-radius:50%;background:#27ae60;display:inline-block"></span>Online
      </div>
      <button class="btn-logout" style="justify-content:center" @click="emit('logout'); mobileMenuOpen = false">
...
function navigate(p) { page.value = p; mobileMenuOpen.value = false }
</script>
